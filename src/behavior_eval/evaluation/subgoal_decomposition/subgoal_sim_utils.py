...
from behavior_eval.evaluation.subgoal_decomposition.constraint_layer import (
    ConstraintPipeline,
    ConstraintFilter,
    RuleBasedConstraintGenerator,
    VLMConstraintGenerator,
    AffordanceValidator,
    ConstraintResult,
)
...
class EvalSubgoalPlan:
    def __init__(self, demo_path:str, plan_path:str, json_format:Optional[bool]=False, constraint_mode: Optional[str] = None) -> None:
        ...
        try:
            ...
            raise e
        self.vocab = Vocab(self.tl_name_mapping, self.tl_relevant_objects)
        self.constraint_mode = (constraint_mode or behavior_eval.subgoal_constraint_mode).lower()
        self.use_constraint_layer = self.constraint_mode != "baseline"
        self.constraint_result: Optional[ConstraintResult] = None
        if self.use_constraint_layer:
    ...
    def _build_env_summary(self) -> Dict[str, Any]:
        return {
            "task": self.task_name,
            "relevant_objects": self.tl_relevant_objects,
            "name_mapping": self.tl_name_mapping,
        }

    def _run_constraint_pipeline(self):
        env_summary = self._build_env_summary()
        generator = (
            VLMConstraintGenerator(self.vocab)
            if self.constraint_mode == "vlm"
            else RuleBasedConstraintGenerator(self.vocab)
        )
        validator = (
            AffordanceValidator(self.vocab)
            if self.constraint_mode in {"vlm", "affordance"}
            else None
        )
        pipeline = ConstraintPipeline(
            generator=generator,
            filterer=ConstraintFilter(self.vocab),
            validator=validator,
        )
        result = pipeline.run(self.subgoal_plan.subgoal_plan, env_summary)
        self.constraint_result = result
        if result.corrected_subgoals:
            self.subgoal_plan.override_subgoal_plan(result.corrected_subgoals)
...
def evaluate_task(
    ...
    """Evaluate a single task plan with optional constraint configuration.

    Args:
        demo_name: Name of the demonstration/task to evaluate.
        plan_path: Path to the LLM plan file.
        constraint_mode: Which constraint pipeline to use. ``None``
            defaults to the global configuration.
        return_metadata: Whether to return constraint metadata (e.g.,
            reprompt statistics) alongside the report.

    Returns:
        The evaluation report tuple. When ``return_metadata`` is ``True``
        the function returns a tuple ``(report, metadata)`` where
        ``metadata`` contains constraint pipeline diagnostics.
    """
    constraint_metadata = None
    try:
        eval_subgoal_plan = EvalSubgoalPlan(
            demo_name, plan_path, constraint_mode=constraint_mode
        )
        report = eval_subgoal_plan.evaluate_subgoal_plan()
        if return_metadata and eval_subgoal_plan.constraint_result is not None:
            constraint_metadata = eval_subgoal_plan.constraint_result.metadata
    except Exception as e:
        ...
        if return_metadata:
            return report, constraint_metadata
        return report

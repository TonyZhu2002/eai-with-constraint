...
class SubgoalPlan:
    ...
    def format_tl_formula(self, subgoal_plan: List[Union[str, Dict[str, Any]]]) -> str:
        return ' then '.join([SubgoalPlanJSON.get_predicate_form(plan) if isinstance(plan, dict) else str(plan) for plan in subgoal_plan])

    def override_subgoal_plan(self, new_subgoal_plan: List[Union[str, Dict[str, Any]]]) -> None:
        '''Override the cached subgoal plan after constraint repairs.'''
        self.subgoal_plan = new_subgoal_plan
        self.subgoal_plan_tl_formula = self.format_tl_formula(new_subgoal_plan)
